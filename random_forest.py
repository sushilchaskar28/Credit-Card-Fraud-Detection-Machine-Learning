...
x_neg = dataset.loc[dataset['Class']==0].sample(frac=1) #Shuffle
...
new_dsArr=[]
for i in range(5):
    new_dsArr.append(pd.concat([x_pos, xnegArr[i]]).sample(frac = 1))  #5 balanced datasets
    
print("Method 4: training using Random Forest....")    
x_set = new_dsArr[0].iloc[:,1:-1].values.tolist() #training on 1st balanced dataset
...
cls_rf = RandomForestClassifier(max_depth=3, random_state=0,n_estimators=10)
...
#Feature selection
cls_fr = RandomForestClassifier(max_depth=3, random_state=0,n_estimators=10, criterion='entropy')
cls_fr.fit(x_train, y_train)
fea_sc= (cls_fr.feature_importances_)
...
x_train, x_test, y_train, y_test = train_test_split(x_set, y_set, test_size=0.30, random_state=1)
clsf = RandomForestClassifier(max_depth=3, random_state=0, n_estimators=10, criterion='entropy')
